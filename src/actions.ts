...
export class Actions implements Disposable {
  ...
  public async openMenu(mode?: string, line?: string, col?: string) {
    ...
    let titles = this.codeActions.map(
      (item) => ` ${item.title.padEnd(width, " ")} `
    );
    width += 2;
    if (
      workspace.getConfiguration(SETTING_SECTION).get("showActionKind", true)
    ) {
      const typeWidth =
        this.codeActions.reduce((pre, cur) => {
          return cur.kind && pre > cur.kind.length
            ? pre
            : (cur.kind && cur.kind.length) || 1;
        }, 0) + 2;
      const types = this.codeActions.map((item) =>
        `[${item.kind || "-"}]`.padStart(typeWidth, " ")
      );
    ...
    const pos = (await this.nvim.call("win_screenpos", winnr)) as [
      number,
      number
    ];
    ...
    await this.createWin(
      buf,
      width,
      Math.min(maxHeight, titles.length),
      anchor
    );
    this.addHighlight(0);
    setTimeout(() => {
  ...
  public async closeMenu() {
    if (this.win) {
      ...
      if (
        this.guiCursor &&
        workspace.getConfiguration(SETTING_SECTION).get("hideCursor", true)
      ) {
        if (gte(workspace.env.version, "0.5.0")) {
          this.nvim.setOption(
            "guicursor",
            `${this.guiCursor},a:ver1-Cursor/lCursor`
          );
        }
  ...
  private async createWin(
    ...
    win.setOption(
      "winhighlight",
      "Normal:Pmenu,FoldColumn:Pmenu,CursorLine:PmenuSel"
    );
    win.setOption("listchars", "trail: ,extends: ");
    win.setCursor([1, 1]);
    if (
      gte(workspace.env.version, "0.5.0") &&
      workspace.getConfiguration(SETTING_SECTION).get("hideCursor", true)
    ) {
      this.nvim.setOption(
        "guicursor",
        `${this.guiCursor},a:ver1-CocCursorTransparent/lCursor`
      );
    }
  ...
  private registerAutocmd() {
    ...
    this.subscriptions.push(
      workspace.registerLocalKeymap(
        ...
        "<cr>",
        async () => {
          const pos = (await this.nvim.callFunction("getcurpos")) as [
            number,
            number,
            number,
            number,
        ...
        if (service.client) {
          ...
          client
            ...
            .then(undefined, (error) => {
              workspace.showMessage(
                `Execute '${command!.command} error: ${error}'`,
                "error"
              );
            });
        }
  ...
  private async getCodeActions(mode?: string) {
    ...
    let diagnostics = diagnosticManager.getDiagnosticsInRange(
      doc.textDocument,
      range
    );
